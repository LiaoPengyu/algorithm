...
int t, n, q, k;
int ans[N], c[N][N], cnt[N], rank[2][N], *rnk, *trnk, pos[N << 1], sum[N], sa[N], tsa[N], p;
string s;
void sorting(int j) {
    memset(sum, 0, sizeof(sum));
    for (int i = 1; i <= n; i++) sum[pos[i + j]]++;
    for (int i = 1; i <= p; i++) sum[i] += sum[i - 1];
    for (int i = 1; i <= n; i++) tsa[sum[pos[i + j]]--] = i;
    memset(sum, 0, sizeof(sum));
    for (int i = 1; i <= n; i++) sum[rnk[i]]++;
    for (int i = 1; i <= p; i++) sum[i] += sum[i - 1];
    for (int i = n; i > 0; i--) sa[sum[rnk[tsa[i]]]--] = tsa[i]; // Desc
}
void init() {
    c[0][0] = 1;
    for (int i = 1; i < N; i++)
